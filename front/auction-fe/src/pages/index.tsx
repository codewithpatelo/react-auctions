...
const Home = () => {
  ...
  useEffect(() => {
    const fetchAuctions = async () => {
      try {
        ...
        // Chequeamos si el usuario se encuentra autentificado
        if (!token) {
          // Si no lo esta, redirigimos a login
          router.push('/login');
          return;
        }
        // LLamamos a la API de BE
  ...
  // Render condicional en base a carga y validación de errores
  ...
  return (
    <div className="p-6">
        ...
        <div className="mt-5 flex lg:ml-4 lg:mt-0">
          ...
          <span className="ml-3 hidden sm:block">
            <Link href="/create-auction">
              <div className="inline-flex items-center rounded-md bg-indigo-500 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-indigo-600">
      ...
      <div className="auctionsContainer">
        <ul className="auctions divide-y divide-gray-200">
            ...
            <Link key={auction.id} href={`/auction/${auction.id}`} passHref>
              <p className="block cursor-pointer hover:bg-gray-100">
                <li className="flex justify-between gap-x-6 py-5">
                  ...
                  <p className="mt-1 truncate text-xs leading-5 text-gray-500"> Current Price: <b>{auction.currentPrice ?? auction.startingPrice}</b></p>
